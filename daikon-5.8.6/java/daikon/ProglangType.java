...
import daikon.chicory.Runtime.BooleanWrap;
...
public final @Interned class ProglangType implements Serializable {
  ...
  public static ProglangType rep_parse(String rep) {
    ...
    } else if (candidate.base == "bigint") { // interned
      return intern("java.math.BigInteger", candidate.dimensions);
    } else {
      return candidate;
    }
  }
  ...
  static final @Interned String BASE_UINT = "uint";
  static final @Interned String BASE_BIGINT = "java.math.BigInteger";
  ...
  public static final ProglangType BIGINT = ProglangType.intern("java.math.BigInteger", 0);
  public static final ProglangType BIGINT_ARRAY = ProglangType.intern("java.math.BigInteger", 1);
  ...
  public final @Nullable @Interned Object parse_value_scalar(
    ...
    } else if (base ==  BASE_BIGINT){
      // return Intern.intern((new BigInteger(value).toString()));
      // //  Object bigint = new BigInteger(value);
      // //  System.out.println("interned bigint");
      // //  return Intern.intern(bigint);
  ...
  public final @Nullable @Interned Object parse_value_array_1d(
    ...
    if (base == BASE_INT) {
      ...
      @Interned BigInteger[] values_bigint = new BigInteger[len];
      for (int i=0; i<len; i++){
        values_bigint[i] = new BigInteger(value_strings[i]);
      }
      return Intern.intern(values_bigint);
    }  
    else if (base == BASE_STRING) {
  ...
  public boolean elementIsBigInt() {
    return ((dimensions == 1) && baseIsBigInt());
  }
  ...
  public boolean isBigInt() {
    // For reptypes, checking against INT is sufficient, rather than
    // calling isIntegral().
    return ((dimensions == 0) && baseIsBigInt());
  }
  
  public boolean baseIsBigInt(){
    return base == BASE_BIGINT;
  }
