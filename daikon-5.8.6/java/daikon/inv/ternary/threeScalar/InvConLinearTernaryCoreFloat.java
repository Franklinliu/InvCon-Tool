...
public final class InvConLinearTernaryCoreFloat implements Serializable, Cloneable {
  // We are Serializable, so we specify a version to allow changes to
  // method signatures without breaking serialization.  If you add or
  // remove fields, you should change this number to the current date.
  static final long serialVersionUID = 20211009L;

  /** Debug tracer. */
  static final Logger debug = Logger.getLogger("daikon.inv.ternary.threeScalar.InvConLinearTernaryCoreFloat");
  ...
  public InvConLinearTernaryCoreFloat(Invariant wrapper) {
    this.wrapper = wrapper;
  }

  @SideEffectFree
  @Override
  public InvConLinearTernaryCoreFloat clone(@GuardSatisfied InvConLinearTernaryCoreFloat this) {
    try {
      InvConLinearTernaryCoreFloat result = (InvConLinearTernaryCoreFloat) super.clone();
  ...
  @Pure
  public boolean isActive(@GuardSatisfied InvConLinearTernaryCoreFloat this) {
  ...
  private boolean try_points_linear(double x, double y, double z) {
    ...
    case NO_CONSTANT_VARIABLES:
      // Ax + By + z = 0
      return Global.fuzzy.eq(0, coefficients[0] * x + coefficients[1]*y + z);
  ...
  private void calc_bi_linear(Point p0, Point p1) {
    ...
    if ((p0.x != p1.x) && (p0.y != p1.y) && (p0.z != p1.z)) {
      ...
      // new version:
      // linear constraint: Ax + By + Cz = 0
      // lets C = 1
      // we have Ax + By + z = 0
      // then we have A=(y0z1-y1z0)/(x0y1-x1y0)
      //              B=(x0z1-x1z0)/(y0x1-y1x0)
      
      // this is a plane
      line_flag = Flag.NO_CONSTANT_VARIABLES;
      // 
      coefficients[0] = (p0.y*p1.z-p1.y*p0.z)/(p0.x*p1.y - p1.x*p0.y);
      coefficients[1] = (p0.x*p1.z-p1.x*p0.z)/(p0.y*p1.x - p1.y*p0.x);
      coefficients[2] = 1;
      coefficients[3] = 0;
  ...
  public boolean enoughSamples(@GuardSatisfied InvConLinearTernaryCoreFloat this) {
    return isActive();
  }
  ...
  public String repr(@GuardSatisfied InvConLinearTernaryCoreFloat this) {
    return "InvConLinearTernaryCoreFloat" + wrapper.varNames() + ": a=" + a
      + ",b=" + b
      + ",c=" + c
      + ",d=" + d
      + ",values_seen=" + values_seen;
  ...
  @Pure
  public boolean isSameFormula(InvConLinearTernaryCoreFloat other) {
  ...
  @Pure
  public boolean isExclusiveFormula(InvConLinearTernaryCoreFloat other) {
    if (!isActive()
        || !other.isActive()) {
      return false;
    }
   ...
   */
  public @Nullable InvConLinearTernaryCoreFloat merge(List<InvConLinearTernaryCoreFloat> cores, Invariant wrapper) {

    // Look for any active planes.  All must define the same plane
    InvConLinearTernaryCoreFloat first = null;
    for (InvConLinearTernaryCoreFloat c : cores) {
      if (!c.isActive()) {
        continue;
      }
      if (first == null) {
    ...
    if (first == null) {
      first = new InvConLinearTernaryCoreFloat(wrapper);
    } else {
