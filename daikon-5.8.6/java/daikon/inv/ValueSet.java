...
public abstract class ValueSet extends LimitedSizeIntSet implements Serializable, Cloneable {
  ...
  public static ValueSet factory(VarInfo var_info) {
    ...
    } else if (rep_type == ProglangType.BIGINT){
      return new ValueSet.ValueSetBigInt(DEFAULT_MAX_VALUES);
    } else if (rep_type == ProglangType.BIGINT_ARRAY){
      return new ValueSet.ValueSetBigIntArray(DEFAULT_MAX_VALUES);
  ...
  public static class ValueSetBigInt extends ValueSet {
    // We are Serializable, so we specify a version to allow changes to
    // method signatures without breaking serialization.  If you add or
    // remove fields, you should change this number to the current date.
    static final long serialVersionUID = 20211031L;

    java.math.BigInteger min_val = java.math.BigInteger.ZERO;
    java.math.BigInteger max_val = (new java.math.BigInteger("2")).pow(256).subtract(new java.math.BigInteger("1"));

    public ValueSetBigInt(int max_values) {
      super(max_values);
    }
    ...
    public void add(Object v1) {
      ...
      if (v1 instanceof java.math.BigInteger){
        val = (java.math.BigInteger) v1;
      }else if (v1 instanceof String){
        val = new java.math.BigInteger((String)v1);
      }else{
        assert(false);
      }
      if (val.compareTo(min_val) < 0) {
        min_val = val;
      }
      if (val.compareTo(max_val) > 0) {
        max_val = val;
      }
      add(UtilPlume.hash(val.toString()));
    }

    @Override
    protected void add_stats(ValueSet other) {
      ValueSetBigInt vs = (ValueSetBigInt) other;
      if (vs.min_val.compareTo(min_val) < 0) {
        min_val = vs.min_val;
      }
      if (vs.max_val.compareTo(max_val) > 0) {
        max_val = vs.max_val;
      }
    }
    ...
    public static class ValueSetBigIntArray extends ValueSet {
    // We are Serializable, so we specify a version to allow changes to
    // method signatures without breaking serialization.  If you add or
    // remove fields, you should change this number to the current date.
    static final long serialVersionUID = 20031017L;
    ...
    public ValueSetBigIntArray(int max_values) {
      super(max_values);
    }
    ...
    public void add(Object v1) {
      assert v1 != null;
      // if (v1 instanceof java.math.BigInteger[])
      java.math.BigInteger[] val = (java.math.BigInteger[]) v1;
      for (int i = 0; i < val.length; i++) {
        if (val[i].compareTo(min_val)<0) {
          min_val = val[i];
        }
        if (val[i].compareTo(max_val) > 0) {
          max_val = val[i];
        }
      }
      elem_cnt += val.length;
      ...
      String[] val_strings = new String[val.length];
      for (int i=0; i < val.length; i++){
        val_strings[i] = val[i].toString();
      }
      // add(UtilPlume.hash((java.math.BigInteger[]) v1));
      add(UtilPlume.hash(val_strings));
    }

    @Override
    protected void add_stats(ValueSet other) {
      ValueSetBigIntArray vs = (ValueSetBigIntArray) other;
      min_val = min_val.min(vs.min_val);
      max_val = max_val.max(vs.max_val);
