...
public abstract class SingleBigIntSequence extends SingleSequence {
  // We are Serializable, so we specify a version to allow changes to
  // method signatures without breaking serialization.  If you add or
  // remove fields, you should change this number to the current date.
  static final long serialVersionUID = 20220302;

  protected SingleBigIntSequence(PptSlice ppt) {
    super(ppt);
  }

  protected @Prototype SingleBigIntSequence() {
    super();
  }
  ...
  public static final boolean valid_types_static(VarInfo[] vis) {
    return ((vis.length == 1)
        && vis[0].file_rep_type.baseIsBigInt()
  ...
  @Override
  public VarInfo var(
      @GuardSatisfied @UnknownInitialization(SingleBigIntSequence.class) SingleBigIntSequence this) {
   ...
   * caller.
   */
  public abstract InvariantStatus add_modified(BigInteger @Interned [] value, int count);

  /** By default, do nothing if the value hasn't been seen yet. Subclasses can override this. */
  public InvariantStatus add_unmodified(BigInteger @Interned [] value, int count) {
    return InvariantStatus.NO_CHANGE;
  }
   ...
   */
  public abstract InvariantStatus check_modified(BigInteger @Interned [] value, int count);

  public InvariantStatus check_unmodified(BigInteger @Interned [] value, int count) {
    return InvariantStatus.NO_CHANGE;
  }
}
