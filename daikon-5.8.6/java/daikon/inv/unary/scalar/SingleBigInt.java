 ...
 */
public abstract class SingleBigInt extends UnaryInvariant {
  // We are Serializable, so we specify a version to allow changes to
  // method signatures without breaking serialization.  If you add or
  // remove fields, you should change this number to the current date.
  static final long serialVersionUID = 20020122L;

  protected SingleBigInt(PptSlice ppt) {
    super(ppt);
  }

  protected @Prototype SingleBigInt() {
    super();
  }

  public VarInfo var(@GuardSatisfied @UnknownInitialization(SingleBigInt.class) SingleBigInt this) {
  ...
  public static final boolean valid_types_static(VarInfo[] vis) {
    return ((vis.length == 1) && vis[0].file_rep_type.isBigInt());
  }
   ...
   * caller.
   */
  public abstract InvariantStatus add_modified(BigInteger value, int count);

  /** By default, do nothing if the value hasn't been seen yet. Subclasses can override this. */
  public InvariantStatus add_unmodified(BigInteger value, int count) {
   ...
   */
  public abstract InvariantStatus check_modified(BigInteger value, int count);

  public InvariantStatus check_unmodified(BigInteger value, int count) {
    return InvariantStatus.NO_CHANGE;
  }
