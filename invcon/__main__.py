...
def execute_command(args):
    ...
    crawler = Crawler(address = args.contract_address, blockchain=args.blockchain, workdir=args.workspace)
    ...
    if not args.crawl_only:
        daikon_main(address = args.contract_address,  workdir=args.workspace,  contractName = results["name"], storageLayoutJson = results["storageLayout_file"],  input_abi = results["abi_file"],  input_state_change = results["allstatechanges_file"] ,  input_tx_receipt = results["tx_decode_file"])
...
def main():
    ...
    parser.add_argument('--cached', dest="cached", action="store_true",
                        help='whethere using previous result, default is false')
